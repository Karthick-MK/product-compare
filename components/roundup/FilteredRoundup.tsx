...
import { RoundupGrid } from './RoundupGrid'
...
export function FilteredRoundup({ products, aiVerdict }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0">
        <span className="text-xs font-mono text-on-surface-variant uppercase tracking-wider flex-shrink-0">Sort</span>
        {[
        ...
        {[{ value: 4, label: '4+ ★' }, { value: 3, label: '3+ ★' }].map(opt => (
          <button key={opt.value}
